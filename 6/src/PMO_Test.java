...
public class PMO_Test {
    ...
    private void add( List<String> list, String row ) {
        ...
        assert row.length() == cols : "BĹÄd: zĹa dĹugoĹÄ ciÄgu wysĹanego do add";
    ...
    private void findStartStop() {
        ...
        assert start != null && stop != null : "WewnÄtrzny bĹÄd testu: Brak pozycji poczÄtkowej i koĹcowej";
    }
    ...
    private List<Position> getPositions(Supplier<PositionInterface[]> dostawca, String opis ) {
        ...
        assertNotNull( solution, "RowiÄzaniem nie moĹźe byc null" );
        assertFalse( Arrays.stream( solution).anyMatch( s -> s == null ), "W tablicy bÄdÄcej rozwiÄzaniem wykryto null" );
    ...
    private char getCharAtSolution( Position position ) {
        ...
        assertTrue( position.col > 0 && position.col < cols, "Kolumna poza mapÄ " + position );
        assertTrue( position.row > 0 && position.row < rows, "Wiersz poza mapÄ " + position );
    ...
    private void testResult( Position position ) {
        assertEquals( '#', getCharAtSolution( position ), "Pozycja poza oczekiwanÄ trasÄ " + position );

        assertEquals( 1, distance( lastVisited, position ), "BĹÄdna odlegĹoĹÄ pomiÄdzy kolejnymi pozycjami trasy");
    ...
    private void testResult(List<Position> result) {
        try {
            ...
            assertTrue( result.size() > 2, "Oczekiwano, Ĺźe rozwiÄzanie bÄdzie zawieraÄ co najmniej 2 pozycje");
            assertEquals( 'S', getCharAtSolution( result.get(0)), "Pierwsza pozycja rozwiÄzania to ani begin ani end" );
            assertEquals( 'S', getCharAtSolution( result.get( result.size()-1)), "Ostatnia pozycja rozwiÄzania to ani begin ani end");

            lastVisited = result.get(0);

            IntStream.range(1,result.size()-1).mapToObj( idx -> result.get(idx) ).forEach( p -> testResult( p ) );
        } catch ( Exception e ) {
            showException( e, "Niezpodziewany bĹÄd w testResult");
        }
    }
    ...
    @ParameterizedTest
    @DisplayName("Brak alternatyw, brak Ĺlepych ulic - trasa najkrĂłtsza")
    ...
    @ParameterizedTest
    @DisplayName("Brak alternatyw, brak Ĺlepych ulic - trasa najĹatfiejsza")
    @MethodSource( "directions")
    ...
    @ParameterizedTest
    @DisplayName("Brak alternatyw, brak Ĺlepych ulic - trasa najszybsza")
    @MethodSource( "directions")
    ...
    @ParameterizedTest
    @DisplayName("SÄ alternatywy - Fastest")
    @MethodSource( "directions")
    ...
    @Test
    @DisplayName("SÄ alternatywy - Shortest - LEFT")
    ...
    @Test
    @DisplayName("SÄ alternatywy - Shortest - RIGHT")
    ...
    @Test
    @DisplayName("SÄ alternatywy - Fastest - LEFT")
    ...
    @Test
    @DisplayName("SÄ alternatywy - Fastest - RIGHT")
    ...
    @Test
    @DisplayName("SÄ alternatywy - Easiest - RIGHT")
    ...
    @Test
    @DisplayName("SÄ alternatywy - Easiest - LEFT")
