...
public class PMO_Test {
    ...
    static {
        ...
        // UWAGA - z uwagi na rosnÄcÄ liczbÄ linii kodu testu i samych testĂłw
        // czÄĹÄ metod ma taryfÄ umieszczonÄ bezpoĹrednio przed
        // nazwÄ metody.
    }
    ...
    // Tak... poniĹźej sporo powielonego kodu. Problem da siÄ rozwiÄzaÄ w bardziej
    // cywilizowany sposĂłb, ale ten jest Ĺatwiejszy do zrozumienia.
    // Osobom zainteresowanym problemem wyjÄtkĂłw w wyraĹźeniach lambda
    // polecam takÄ stronÄ: https://www.baeldung.com/java-lambda-exceptions
    ...
    private void add(Point point, boolean exceptionExpected ) {
        if (!exceptionExpected) {
            ...
            assertThrows(WrongNumberOfDimensionsException.class, () -> gs.add(point), "Oczekiwano wyjÄtku WrongNumberOfDimensionsException");
        }
    }
    ...
    private void remove(Point point, boolean exceptionExpected ) {
        if (!exceptionExpected) {
            ...
            assertThrows(WrongArgumentException.class, () -> gs.remove(point), "Oczekiwano wyjÄtku WrongArgumentException" );
        }
    }
    ...
    private <T extends Throwable> void addAfter(Point point, Point secondPoint, Class< T >  expected ) {
        if (expected == null) {
            ...
            assertThrows(expected, () -> gs.addAfter(point,secondPoint),
                    "Oczekiwano wyjÄtku " + exceptionName.get(expected));
        }
    }
    ...
    public void getByPosition( java.util.List<java.lang.Integer> positions, Point expected, boolean exceptionExpected ) {
        if ( exceptionExpected ) {
            assertThrows(WrongNumberOfDimensionsException.class, () -> gs.getByPosition(positions),
                    "Oczekiwano wyjÄtku WrongNumberOfDimensionsException");
        } else {
            try {
                Optional<Point> result = gs.getByPosition(positions);
                assertNotNull( result, "Wynikiem getByPosition nigdy nie moĹźe byÄ null");
                if ( expected == null ) {
                    assertFalse( result.isPresent(), "Oczekiwano, Ĺźe punkt nie zostanie zwrĂłcony");
                } else {
                    assertTrue( result.isPresent(), "Oczekiwano zwrĂłcenia punktu, a jest null");
                    assertEquals( expected, result.get(), "Oczekiwano zwrocenia innego punktu");
                    assertTrue( expected==result.get(), "Oczekiwano, Ĺźe zostanie zwrĂłcony punkt ostatnio dodany");
                }
            } catch (Exception e) {
    ...
    @Test
    @DisplayName("Test wyjÄtku w add")
    ...
    @Test
    @DisplayName("Test addAfter - wyjÄtek WrongArgumentException")
    ...
    @Test
    @DisplayName("Test addAfter - wyjÄtek WrongNumberOfDimensionsException")
    ...
    @Test
    @DisplayName("Test addBefore - wyjÄtek WrongArgumentException")
    ...
    @Test
    @DisplayName("Test addBefore - wyjÄtek WrongNumberOfDimensionsException")
    ...
    public void getSetTest() {
        ...
        assertIterableEquals( expected, result, "Oczekiwano innej zawartoĹci zbioru");
    }
